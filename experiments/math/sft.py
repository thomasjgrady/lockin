...
model = Llama.from_meta_checkpoint(checkpoint_dir, mesh, init_fsdp=True)
...
pbar = tqdm(loader, desc="running benchmark...", disable=dist.get_rank() != 0)

for batch in pbar:

    batch: list[list[ChatMessage]] = [x[:-1] + [ChatMessage(role="assistant", content="")] for x in batch]
    ground_truth_responses = [x[-1].content for x in batch]
    ...
    batch_results = [
        {
            "prompt": [m.model_dump(mode="python") for m in b],
